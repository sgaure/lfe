...
static void invertfactor(FACTOR *f, int N)
{
  ...
  for (i = 0; i < N; i++)
  {
    int gp = f->group[i];
    if (gp < 1)
      error("Factors can not have missing levels");
    f->ii[gp]++;
  }
...
FACTOR **makefactors(SEXP flist, int allowmissing, double *weights)
{
  ...
  if (!oneiter)
  {
    ...
    if (isLogical(Roneiter))
      oneiter = LOGICAL(Roneiter)[0];
  }
  ...
  for (int i = 0; i < LENGTH(flist); i++)
  {
    ...
    if (f->nlevels <= 0)
      error("factor %d in list has no levels\n", i + 1);
    f->oneiter = oneiter;
  ...
  for (int i = 0; i < truefac; i++)
  {
    ...
    for (int j = 0; j < N; j++)
    {
      ...
      if (f->group[j] > 0)
      {
        double w = (f->x == NULL) ? (weights == NULL ? 1.0 : weights[j]) : (weights == NULL ? f->x[j] : f->x[j] * weights[j]);
...
SEXP MY_wwcomp(SEXP flist)
{
  ...
  numfac = LENGTH(flist);
  if (numfac < 2)
    error("At least two factors must be specified");
  ...
  for (int i = 0; i < N; i++)
    levsize[fac[i] - 1] = levsize[fac[i] - 1] + 1;
