...
SEXP MY_kaczmarz(SEXP flist, SEXP vlist, SEXP Reps, SEXP initial, SEXP Rcores)
{
  ...
  for (int i = 0; i < listlen; i++)
  {
    ...
    if (!isReal(elt))
    {
      elt = PROTECT(coerceVector(elt, REALSXP));
      protectcount++;
    }
    if (!isMatrix(elt))
    {
  ...
  arg.running = numthr;
#ifdef HAVE_SEM
  if (sem_init(&arg.finished, 0, 0) != 0)
    error("sem_init failed, errno=%d", errno);
#endif
  arg.lock = &lock;
#endif
#endif
  ...
  if (amiss != 0)
    arg.work[0] = (mysize_t *)((uintptr_t)arg.work[0] + sizeof(double) - amiss);
  ...
  for (thr = 0; thr < numthr; thr++)
  {
    ...
    if (amiss != 0)
      arg.work[thr] = (mysize_t *)((uintptr_t)arg.work[thr] + sizeof(double) - amiss);

#ifdef WIN
    threads[thr] = CreateThread(NULL, 0, kaczmarz_thr, &arg, 0, &threadids[thr]);
    if (0 == threads[thr])
      error("Failed to create kaczmarz thread");
#else
    int stat = pthread_create(&threads[thr], NULL, kaczmarz_thr, &arg);
    if (0 != stat)
      error("Failed to create kaczmarz thread, stat=%d", stat);
#endif
  }
  /* wait for completion */
